...
fn my_sql_row_to_owm(row: &mysql_async::Row) -> Result<ObjectWithMetadata, DbError> {
    let id: String = row
        .get(0)
        .ok_or_else(|| DbError::ConversionError(String::from("missing id").into()))?;
    let object_json: String = row
        .get(1)
        .ok_or_else(|| DbError::ConversionError(String::from("missing object").into()))?;
    ...
    let object: Object = serde_json::from_str(&object_json).map_err(|e| {
        DbError::ConversionError(format!("failed deserializing the object: {e}").into())
    })?;
    let attributes: Attributes = serde_json::from_value(attrs_json).map_err(|e| {
        DbError::ConversionError(format!("failed deserializing the Attributes: {e}").into())
    })?;
...
// Removed HasDatabase marker impl; not needed.
...
impl MySqlPool {
    pub(crate) async fn instantiate(
        ...
        let _max_conns: usize = max_connections
